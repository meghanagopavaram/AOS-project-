...
def write(ftp, childServ, username, MAINSERVERHOST, MAINSERVERPORT):
    ...
    if enc_local_name in filenames:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            sock.connect((MAINSERVERHOST, MAINSERVERPORT))
            sock.sendall(bytes("getlockedfiles", encoding="utf-8"))
            received = sock.recv(1024)
            print(received.decode('utf-8'))
            for file in received.decode('utf-8').split(";"):
                if file == enc_local_name:
                    print("The requested file is currently locked. Please try again after sometime!")
                    sock.close()
                    return
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            sock.connect((MAINSERVERHOST, MAINSERVERPORT))
            sock.sendall(bytes("unlockfile:"+enc_local_name+"\n", "utf-8"))
